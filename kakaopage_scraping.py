...
try:
    ...
    print("db sql문 SELECT COUNT(id) FROM kakaopage_product 오류")
...
for num in range(1, social_Number - db_total_productNum + 1) :
    ...
    socialList_id_visitor.append((int(social_Id),int(social_Visitor)))
    mul = 1
    
#   title, category, author, keyword, content
try:
    with db.cursor() as cursor :
        for social_Id, social_Visitor in socialList_id_visitor :
