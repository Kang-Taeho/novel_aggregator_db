...
CREATE TABLE IF NOT EXISTS novels (
  ...
  normalized_title    VARCHAR(300) GENERATED ALWAYS AS (LOWER(TRIM(title))) STORED,
  normalized_author   VARCHAR(200) GENERATED ALWAYS AS (LOWER(TRIM(COALESCE(author_name,'')))) STORED,
...
CREATE TABLE IF NOT EXISTS novel_sources (
  ...
  platform_item_id   VARCHAR(191) NOT NULL,
  episode_count      INT UNSIGNED NULL,
  last_episode_date  DATETIME(6) NULL,
